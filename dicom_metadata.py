        ...
        for key, value in sorted(flywheel_dicom_header.items()):
            ...
            if key not in local_dicom_header and vr != "SQ":
                ...
                if local_dicom_header[key] != flywheel_dicom_header[key]:
                    ...
                    log_message += "Any difference in {} is not accounted for.".format(
                        key
                    )
                ...
                log_message = (
                    "Other Float (OF) DICOM Tags are not modified by this gear\n"
                )
                log_message += f"{key} will not be inserted into the dicom file(s)"
                log.warning(log_message)
            else:
                ...
                if local_dicom_header[key] != flywheel_dicom_header[key]:
                    ...
                    if (
                        not isinstance(local_dicom_header[key], list)
                        and isinstance(flywheel_dicom_header[key], list)
                        and local_dicom_header[key] == flywheel_dicom_header[key][0]
                    ):
                        log.info(
...
def dcm_dict_is_representative(dcm_data_dict, use_rawdatastorage=False):
    ...
    if (
        dcm_data_dict["size"] > 0
        and dcm_data_dict["header"]
        and not dcm_data_dict["pydicom_exception"]
    ):
        ...
        if (
            dcm_data_dict["header"].get("SOPClassUID") == "Raw Data Storage"
            and not use_rawdatastorage
        ):
            log.warning(
...
def select_matching_file(file_list, flywheel_header_dict):
    ...
    if len(file_list) == 1:
        path = file_list[0]
        log.info("Only one DICOM file, using %s to compare to flywheel header", path)
        return path

    if all([bool(flywheel_header_dict.get(tag) is None) for tag in instance_tag_list]):
        log.warning("Could not match file to Flywheel header - missing match tags.")
        return None
    for path in file_list:
        try:
            ...
            if all(
                [
                    header_inst_dict.get(tag) == flywheel_header_dict.get(tag)
            ...
            log.error("An exception was raised when parsing %s", path, exc_info=True)
            continue
    warn_str = (
