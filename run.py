...
from dicom_metadata import (
    compare_dicom_headers,
    dicom_header_extract,
...
def _copy_files_from_session(fw, from_session, to_session):
    ...
    """
    log.debug(
        f"Copying session {from_session.label} file attachments to session {to_session.label}"
    )
    ...
    for session_file in session_files:
        ...
        if session_file.type == "dicom":
            log.warning(
                f"File {session_file.name} is type DICOM.\n"
        ...
        try:
            ...
            download_file = os.path.join(
                "/tmp", get_sanitized_filename(session_file.name)
            )
            ...
            exported_file = upload_file_with_metadata(
                fw, session_file, to_session, download_file
            )
            if exported_file is None:
                raise RuntimeError(
                    f"Failed to export file {os.path.basename(download_file)} to {to_session.label}"
                )
...
def _update_file_metadata(fw, orig_f, new_f):
    ...
    if orig_f.classification:
        ...
        if validate_classification(
            fw, orig_f.modality, classification_dict, orig_f.name
        ):

            log.debug(
                "Updating classification to %s for %s"
                % (classification_dict, new_f.name)
            )
            new_f.update_classification(classification_dict)
        else:
            log.error(
                "Not updating classification to %s for %s"
                % (orig_f.classification, new_f.name)
            )
    if orig_f.info:
...
def _archive_session(fw, session, archive_project):
    ...
    subject, _ = _find_or_create_subject(fw, session, archive_project, subj_code)
...
def _export_dicom(dicom_file, tmp_dir, acquisition, session, subject, project, config):
    ...
    if "header" in dicom_file.info:
        ...
        local_dicom_header = dicom_header_extract(
            dicom_file_path, flywheel_dicom_header
        )
        ...
        log.warning(
            "WARNING: Flywheel DICOM does not have DICOM header at info.header.dicom!"
        )
        if config["map_flywheel_to_dicom"]:
            log.warning(
                "WARNING! map_flywheel_to_dicom is True, however there is no DICOM header information in Flywheel. Please run.py GRP-3 (medatadata extraction) to read DICOM header data into Flywheel."
            )
    ...
    if config["map_flywheel_to_dicom"]:
        ...
        fields_map = {
            ...
            "PatientAge": "%sY" % str(session.get("age_years"))
    ...
    upload_file_path = _modify_dicom_archive(
        dicom_file_path, update_keys, flywheel_dicom_header, dicom_path_list, tmp_dir
    )

    return upload_file_path, flywheel_dicom_header


def _modify_dicom_archive(
    dicom_file_path, update_keys, flywheel_dicom_header, dicom_file_list, tmp_dir
):
    ...
    update_dict = {
        key: flywheel_dicom_header.get(key)
        for key in update_keys
        if key in flywheel_dicom_header
    }
    ...
    log.info(
        "Updating {} keys in {} dicom files...".format(
            len(update_keys), len(dicom_files)
        )
    )
    ...
    if error_files_list:
        raise RuntimeError(
            f"Failed to edit {dicom_file_path} files: {error_files_list}"
        )
    ...
    if is_zip:
        ...
        modified_dicom_file_path = _create_archive(
            dicom_base_folder, os.path.basename(dicom_base_folder), dicom_files
        )
    else:
            ...
            if key in classification_dict:
                for val in values:
                    ...
                    if val not in classification_dict[key]:
                        log.error(
                            'For %s, modality "%s", "%s" is not valid for "%s".'
                ...
                log.error(
                    'For %s, modality "%s", "%s" is not in classification schema.'
...
def get_file_classification(fw_client, file_object, file_modality, export_file_name):
    ...
    if file_classification:
        ...
        if validate_classification(
            fw_client, file_modality, file_classification, export_file_name
        ):

            log.debug(
                "File %s classification will be set to %s"
                % (export_file_name, file_classification)
            )

        else:
            log.error(
                "Classification %s is invalid. Classification will not be set for file %s"
                % (file_classification, export_file_name)
            )
            file_classification = None
    else:
...
def format_file_metadata_upload_str(
    fw_client, file_object, export_file_name, new_dicom_header=None
):
    """
    ...
    file_classification = get_file_classification(
        fw_client, file_object, file_modality, export_file_name
    )
...
def upload_file_with_metadata(
    ...
    metadata_str = format_file_metadata_upload_str(
        fw_client, origin_file, export_file_name, flywheel_dicom_header
    )
    ...
    while attempt < max_attempts:
        attempt += 1
        fw_client.upload_file_to_container(
            destination_container.id, local_file_path, metadata=metadata_str
        )
...
def _export_files(
    fw, acquisition, export_acquisition, session, subject, project, config
):
    ...
    for f in acquisition.files:
        log.info(
            "Exporting %s/%s/%s/%s/%s..."
            % (project.label, subject.label, session.label, acquisition.label, f.name)
        )

        with tempfile.TemporaryDirectory() as temp_dir:

            if f.type == "dicom":
                upload_file_path, flywheel_dicom_header = _export_dicom(
                    f, temp_dir, acquisition, session, subject, project, config
                )
            else:
                upload_file_path = os.path.join(
                    temp_dir, get_sanitized_filename(f.name)
                )
            ...
            # Upload the file to the export_acquisition
            exported_file = upload_file_with_metadata(
                fw, f, export_acquisition, upload_file_path, flywheel_dicom_header
            )
            if exported_file is None:
                raise RuntimeError(
                    f"Failed to export file {export_file_name} to {export_acquisition.label}"
                )
...
def main(context):
    ...
    export_project, archive_project, destination = validate_context(context)

    # Track what is being created
    creatio = []
    creatio_instance = {"container": None, "id": None, "new": None}
    try:
        ########################################################################
        ...
        sub_export["origin_path"] = "{}/{}/{}".format(
            project.group, project.label, subject_code
        )
        sub_export["export_path"] = "{}/{}/{}".format(
            export_project.group, export_project.label, subject_code
        )
        if archive_project:
            sub_export["archive_path"] = "{}/{}/{}".format(
                archive_project.group, archive_project.label, subject_code
            )

        subj, created = _find_or_create_subject(
            fw=fw, session=session, project=export_project, subject_code=subject.code
        )
        if created:
            log_str = (
                f"Created subject {subject_code} in project " f"{export_project.label}"
            )
            log.info(log_str)
        ...
        log.info(
            "CREATING SESSION CONTAINER {} IN {}/{}".format(
                session.label, export_project.label, subject.label
            )
        )
        ...
        if archive_project:
            session_export["archive_path"] = "{}/{}/{}/{}".format(
                archive_project.group,
                archive_project.label,
                subject.code,
                session.label,
            )
        ...
        for acq in session.acquisitions():
            ...
            acquisition_export["export_path"] = "{}/{}/{}/{}/{}".format(
                export_project.group,
                export_project.label,
                subject.code,
                session.label,
                acq.label,
            )
            ...
            _export_files(
                fw, acq, export_acquisition, session, subject, project, context.config
            )
        ...
        with open(export_log, "w") as lf:
            csvwriter = csv.writer(
                lf, delimiter=",", quotechar='"', quoting=csv.QUOTE_MINIMAL
            )
            csvwriter.writerow(
                [
                    "Container",
...
if __name__ == "__main__":
    ...
    with flywheel_gear_toolkit.GearToolkitContext() as context:
        if context.config.get("log_debug"):
            # TODO: This should redner INFO and WARNING on stdout, and all others on stderr
            # We could also make our own class that calls configure_logging in __init__()
            flywheel_gear_toolkit.configure_logging(default_config_name="debug")
        log.info(f"{context.config}")
        main(context)
        log.info("DONE!")
        os._exit(0)
