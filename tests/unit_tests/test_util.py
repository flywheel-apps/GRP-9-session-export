...
def test_hash_value():
    assert hash_value("0123456789") == "84d89877f0d4041efb6bf91a16f0248f2fd573e6af05c19f96bedb9f882f7882"
    assert hash_value("0123456789", output_format="dec") == "13221615211924021243025110724926222403614347213115230175519315915019021915913647120130"
    assert hash_value("0123456789", salt="0123456789") == "4e76ad8354461437c04ef9b9b242540b6406d782ff2c3fb28afdab5b423f88fe"
...
def test_false_if_exc_is_timeout():
    ...
    assert all([false_if_exc_is_timeout(ApiException(status=x)) for x in [404, 403, 400]])
    assert not any([false_if_exc_is_timeout(ApiException(status=x)) for x in [500, 502, 504]])
...
def test_false_if_exc_is_timeout_or_sub_exists():
    ...
    assert all([false_if_exc_is_timeout_or_sub_exists(ApiException(status=x)) for x in [404, 403, 400]])
    assert not any([false_if_exc_is_timeout_or_sub_exists(ApiException(status=x)) for x in [500, 502, 504]])
    ...
    assert not any([false_if_exc_is_timeout_or_sub_exists(set_detail(ApiException(status=x), has_sub_detail)) for x in [409, 422]])
    assert all([false_if_exc_is_timeout_or_sub_exists(ApiException(status=x)) for x in [409, 422]])